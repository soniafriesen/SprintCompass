...
const Subtask = (props) => {
  ...
  const assignedmemberOnChange = (e, selection) => {
    selection
      ? setState({ assignedmember: selection.firstname + " " + selection.lastname })
  ...
  const fetchSubtaskInfo = async () => {
    try {
      ...
      let response = await fetch(GRAPHURL, {
        ...
        body: JSON.stringify({
          query:
            "{getallsubtasks{subtask,userstory,assignedmember,status}}",
        }),
      });
      let payload = await response.json();
      props.dataFromChild(
  ...
  const addSubtask = async () => {
    try {
      let response = await fetch(GRAPHURL, {
        ...
        headers: { "Content-Type": "application/json; charset=utf-8" },
        body: JSON.stringify({
          query: ` mutation { addsubtask ( userstory: "${state.userstory}", subtask: "${state.subtask}" )
                            { userstory, subtask, assignedmember, status } } `,
        }),
      });
      let payload = await response.json();
      props.dataFromChild(
        `added info for ${payload.data.addsubtask.userstory} project`
      );
      setState({
        userstory: "",
        subtask: "",
                ...
                <TableRow key="fillable">
                  ...
                  >
                    <Autocomplete
                      ...
                      style={{ width: 600, height: 20, paddingTop: "2vh", paddingBottom: "5vh" }}
                      renderInput={params => (
                        <TextField
                          {...params}
                          label="user story"
                  ...
                  >
                    <Autocomplete
                      ...
                      style={{ width: 150, height: 20, paddingTop: "2vh", paddingBottom: "5vh" }}
                      renderInput={params => (
                        <TextField
                          {...params}
                          label="member"
