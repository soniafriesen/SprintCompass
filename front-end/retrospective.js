...
const Retrospective = (props) => {
    ...
    const buttonEnabled = state.project === undefined || state.project === ""
        ...
        || state.userstory === undefined || state.userstory === ""
        || state.subtask === undefined || state.subtask === ""
        || state.originalhours === undefined || state.originalhours === ""
        || state.actualhours === undefined || state.actualhours === ""
        || state.reestimatehours === undefined || state.reestimatehours === "";
    const updateButtonEnabled = state.project === undefined || state.project === ""
        || state.sprint === undefined || state.sprint === ""
        || state.userstory === undefined || state.userstory === ""
        || state.subtask === undefined || state.subtask === ""
        || state.originalhours === undefined || state.originalhours === ""
    ...
    const userstoryOnChange = (e, selection) => {
        selection
            ...
            : setState({ userstory: "" });
        selection
            ? fetchSubtasks(selection.userstory)
            : fetchSubtasks("");
    };
        ...
        try {
            ...
            let response = await fetch(GRAPHURL, {
                ...
                body: JSON.stringify({
                  ...
                  { project,sprint,userstory,subtask,storypoints,originalhours,actualhours,reestimatehours } }` })
            });
            let payload = await response.json();
            props.dataFromChild(`found ${payload.data.getallretroforproject.length} retrospectives`);
    ...
    const fetchSprints = async (project) => {
        try {
            let response = await fetch(GRAPHURL, {
                method: "POST",
                headers: { "Content-Type": "application/json; charset=utf-8" },
                body: JSON.stringify({ query: `{getallsprintsperproject(project:"${project}"){sprint,numberofuserstory,project,team,storypoints}}` })
            });
            let payload = await response.json();
            props.dataFromChild(`found ${payload.data.getallsprintsperproject.length} sprints`);
            setState({
    ...
    const fetchStories = async () => {
        try {
            let response = await fetch(GRAPHURL, {
                method: "POST",
                headers: { "Content-Type": "application/json; charset=utf-8" },
                body: JSON.stringify({ query: "{getalluserstory{userstory,sprint,status,numberofsubtasks,userpoints,assignedmember}}" })
            });
            let payload = await response.json();
            props.dataFromChild(`found ${payload.data.getalluserstory.length} stories`);
            setState({
    ...
    const fetchSubtasks = async (userstory) => {
        try {
            ...
            let payload = await response.json();
            props.dataFromChild(`found ${payload.data.getallsubtaskbyuserstory.length} subtasks`);
            setState({
    ...
    const addRetrospective = async () => {
        try {
            let response = await fetch(GRAPHURL, {
                ...
                headers: { "Content-Type": "application/json; charset=utf-8" },
                body: JSON.stringify({
                    query: ` mutation { addsprintretrospective ( project: "${state.project}", sprint: "${state.sprint}", userstory: "${state.userstory}", subtask: "${state.subtask}", originalhours: ${state.originalhours} actualhours: ${state.actualhours}, reestimatehours: ${state.reestimatehours} )
                            { project, sprint, userstory, subtask, storypoints, originalhours, actualhours, reestimatehours } } `
                })
            });
            setState({
            ...
            <Card>
                ...
                <CardContent>
                    <Autocomplete
                        ...
                        style={{ width: 350, height: 20, paddingTop: "2vh", paddingBottom: "10vh" }}
                        renderInput={params => (
                            <TextField
                                {...params}
                                label="project info"
                                ...
                                <TableRow key="fillable">
                                    ...
                                    <TableCell component="th" scope="row" style={{ color: theme.palette.primary.main, textAlign: "center", fontSize: 14 }}>
                                        <Autocomplete
                                            ...
                                            style={{ width: 175, height: 20, paddingTop: "2vh", paddingBottom: "10vh" }}
                                            renderInput={params => (
                                                <TextField
                                                    {...params}
                                                    label="sprints"
                                    ...
                                    <TableCell component="th" scope="row" style={{ color: theme.palette.primary.main, textAlign: "center", fontSize: 14 }}>
                                        <Autocomplete
                                            ...
                                            style={{ width: 375, height: 20, paddingTop: "2vh", paddingBottom: "10vh" }}
                                            renderInput={params => (
                                                <TextField
                                                    {...params}
                                                    label="user stories"
