...
const ProjectInfo = (props) => {
    ...
    const buttonEnabled = state.name === undefined || state.name === ""
        || state.description === undefined || state.description === ""
        || state.numberofteammembers === undefined || state.numberofteammembers === ""
        || state.teamname === undefined || state.teamname === ""
        || state.numberofsprints === undefined || state.numberofsprints === ""
        || state.velocity === undefined || state.velocity === ""
        || state.storypointsconversion === undefined || state.storypointsconversion === ""
        || state.costperhour === undefined || state.costperhour === "";
    const deleteButtonEnabled = state.name === undefined || state.name === ""
        || state.teamname === undefined || state.teamname === "";
    ...
    const editSelectOnChange = (e, selection) => {
        selection
            ? setState({ name: selection.name, description: selection.description, teamname: selection.teamname, numberofteammembers: selection.numberofteammembers, numberofsprints: selection.numberofsprints, velocity: selection.velocity, storypointsconversion: selection.storypointsconversion, costperhour: selection.costperhour })
            : setState({
                name: "",
                description: "",
                teamname: "",
    ...
    const fetchProjectInfo = async () => {
        try {
            ...
            console.log(payload.data.getallproject);
            setState({
    ...
    const addProjectInfo = async () => {
        try {
            let response = await fetch(GRAPHURL, {
                ...
                headers: { "Content-Type": "application/json; charset=utf-8" },
                body: JSON.stringify({
                    query: ` mutation { addproject ( name: "${state.name}", description: "${state.description}", teamname: "${state.teamname}", numberofteammembers: ${state.numberofteammembers}, numberofsprints: ${state.numberofsprints}, velocity: ${state.velocity}, storypointsconversion: ${state.storypointsconversion}, costperhour: ${state.costperhour} )
                            { name, description, teamname, numberofteammembers, numberofsprints, velocity, storypointsconversion, costperhour } } `
                })
            });
            let payload = await response.json();
            props.dataFromChild(`added info for ${payload.data.addproject.name} project`);
            setState({
                name: "",
                description: "",
                teamname: "",
    ...
    const deleteProjectInfo = async () => {
        try {
            ...
            let response = await fetch(GRAPHURL, {
                method: "POST",
                headers: { "Content-Type": "application/json; charset=utf-8" },
                body: JSON.stringify({ query: ` { deletespecificproject ( name:"${state.name}", teamname:"${state.teamname}" ) } ` })
            });
            let payload = await response.json();
