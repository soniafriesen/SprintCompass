...
const MemberInfo = (props) => {
    ...
    const buttonEnabled = state.team === undefined || state.team === ""
        || state.firstname === undefined || state.firstname === ""
        || state.lastname === undefined || state.lastname === ""
        || state.email === undefined || state.email === "";
    const deleteButtonEnabled = state.lastname === undefined || state.lastname === ""
        || state.email === undefined || state.email === "";
    ...
    const editSelectOnChange = (e, selection) => {
        selection
            ? setState({ team: selection.team, firstname: selection.firstname, lastname: selection.lastname, email: selection.email })
            : setState({
                team: "",
                firstname: "",
                lastname: "",
    ...
    const fetchMemberInfo = async () => {
        try {
            ...
            let response = await fetch(GRAPHURL, {
                ...
                body: JSON.stringify({ query: "{getallteammembers{team,firstname,lastname,email}}" })
            });
            let payload = await response.json();
            props.dataFromChild(`found ${payload.data.getallteammembers.length} teammembers`);
    ...
    const addMemberInfo = async () => {
        try {
            ...
            let payload = await response.json();
            props.dataFromChild(`added info for ${payload.data.addteammember.name} project`);
            setState({
                team: "",
                firstname: "",
                lastname: "",
    ...
    const deleteMemberInfo = async () => {
        try {
            ...
            let response = await fetch(GRAPHURL, {
                method: "POST",
                headers: { "Content-Type": "application/json; charset=utf-8" },
                body: JSON.stringify({ query: ` { deleteamember ( lastname: "${state.lastname}", email: "${state.email}" ) } ` })
            });
            let payload = await response.json();
                ...
                <CardContent>
                    ...
                    <div align="center">
                        <Autocomplete
                            ...
                            getOptionLabel={(option) => option.firstname + " " + option.lastname + "; Team: " + option.team}
